...
### Modificato
- **La pagina portafoglio si divide in due schede: Posizioni e Portafoglio.** In *Posizioni* restano le posizioni aperte, gli ordini pendenti e la barra del capitale; in *Portafoglio* passano il calendario del P&L giornaliero e i titoli che segui a mano con l'analisi Damodaran. Tenerli uno sotto l'altro voleva dire scorrere mezza pagina per passare dall'uno all'altro. La scheda scelta viene ricordata fra una visita e l'altra.
  - I grafici dell'esposizione nascono ora dentro un pannello nascosto, dove Chart.js misura un contenitore alto zero e li disegnerebbe schiacciati: vengono ridimensionati all'apertura della scheda. Serve solo la prima volta, ma quella volta serve sempre.

### Tecnico
- Il payload delle posizioni esce dal route handler in `_ibkr_holdings_payload()`, così `?diag=holdings` sul cron mostra la risposta *esatta* che riceve la pagina invece di una ricostruzione che diverge proprio dove serve guardare.
- `?diag=pos` spiega perché una posizione mostrerebbe il Gain/Loss a trattino: quel numero, per le posizioni che arrivano dal Flex, non è un dato di IBKR ma è ricalcolato col listino, e la catena ha tre anelli — simbolo FMP risolto dalla mappa earnings, quotazione, prezzo di carico. Se ne salta uno il numero sparisce, e dall'esterno i tre casi si somigliano.
